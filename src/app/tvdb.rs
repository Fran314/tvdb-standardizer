...
pub async fn api_search_movie(
    tvdb_auth_token: String,
    search_string: String,
    year: String,
) -> Result<Vec<Entry>, Box<dyn std::error::Error>> {
    api_search(tvdb_auth_token, "movie".to_owned(), search_string, year).await
}
pub async fn api_search_series(
    tvdb_auth_token: String,
    search_string: String,
    year: String,
) -> Result<Vec<Entry>, Box<dyn std::error::Error>> {
...
async fn api_search(
    ...
    let res: Response<Vec<RawEntry>> = client
        .get(url)
        .header("accept", "application/json")
        .header(
            "Authorization",
            "Bearer ".to_owned() + tvdb_auth_token.as_ref(),
        )
        .send()
        .await?
        .json()
        .await?;
    let data: Vec<Entry> = res
        .data
        .into_iter()
...
pub async fn api_episode(
    tvdb_auth_token: impl AsRef<str>,
    tvdb_id: impl AsRef<str>,
    season: impl AsRef<str>,
